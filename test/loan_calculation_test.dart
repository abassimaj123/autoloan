...
void main() {
  ...
  const expectedLoanAmt   = caVehiclePrice + expectedTaxON - caDownPayment; // 28900
  ...
  test('TEST 2 — Monthly payment ≈ \$584.61 (no insurance)', () {
    ...
    expect(r.monthlyPayment, inInclusiveRange(583.0, 586.0),
  ...
  test('TEST 3 — Total interest ≈ \$6,176 (no insurance)', () {
    ...
    expect(r.totalInterest, inInclusiveRange(6000.0, 6400.0),
  ...
  test('TEST 5 — Total cost = price + tax + interest + insurance (no duplicate down)', () {
    final r = CACalculation.calculate(
      vehiclePrice: caVehiclePrice,
      downPayment:  caDownPayment,
      annualRate:   caAnnualRate,
    ...
    final expected = caVehiclePrice + r.taxAmount + r.totalInterest + r.insuranceTotal;
    expect(r.totalCost, closeTo(expected, 0.01));
    expect(r.totalCost, inInclusiveRange(41000.0, 43000.0),
    ...
    final doubleCountedDP = caVehiclePrice + r.taxAmount + caDownPayment + r.totalInterest + r.insuranceTotal;
  ...
  test('TEST 6 — Real cost (via financing) = totalCost - downPayment', () {
    ...
    expect(realCost, closeTo(r.loanAmount + r.totalInterest + r.insuranceTotal, 0.01));
    expect(realCost, inInclusiveRange(36000.0, 38000.0),
  ...
  test('TEST 7 — Bi-weekly uses own formula r=rate/26, n=years×26', () {
    ...
    expect(r.baseLoanBiWeekly, closeTo(expectedBiWeekly, 0.0001),
  ...
  test('TEST 7b — Insurance bi-weekly = insuranceMonthly × 12/26', () {
    ...
    expect(r.insuranceBiWeekly, closeTo(13.08, 0.05),
        reason: '28.33 × 12/26 ≈ \$13.08');
    expect(r.biWeeklyPayment, closeTo(r.baseLoanBiWeekly + r.insuranceBiWeekly, 0.0001));
  });
  ...
  test('TEST 7c — Monthly with insurance ≈ \$612.94', () {
    ...
    expect(r.monthlyPayment, inInclusiveRange(611.0, 615.0),
  ...
  test('TEST 9 — Formula gives same value regardless of rounding path', () {
    ...
    expect(r.monthlyPayment, closeTo(recomputed, 0.0001),
  ...
  test('TEST 10 — Amortization month 1 (interest / principal / balance)', () {
    ...
    expect(interest1,  closeTo(190.25, 0.10), reason: '28900 × 0.0065833 ≈ \$190.25');
    expect(principal1, closeTo(394.36, 0.10), reason: 'payment - interest ≈ \$394.36');
    expect(balance1,   closeTo(28505.64, 0.20), reason: '28900 - 394.36 ≈ \$28,505.64');
  });
  ...
  test('TEST 11 — Amortization: last month balance ≈ \$0.00', () {
    ...
    expect(balance, closeTo(0.0, 1.0), reason: 'Final balance must clear to \$0');
  });
  ...
  test('TEST 14 — US credit score adjustments ±%', () {
    calc(CreditScore cs) => USCalculation.calculate(
      vehiclePrice: 30000, tradeInValue: 0, downPayment: 0,
      dealerFees: 0, salesTaxPercent: 0,
      annualRate: 7.9, termMonths: 60, creditScore: cs,
    );

    expect(calc(CreditScore.excellent).effectiveRate, closeTo(6.40, 0.001),
        reason: '7.90 - 1.5 = 6.40%');
    expect(calc(CreditScore.good).effectiveRate, closeTo(7.40, 0.001),
        reason: '7.90 - 0.5 = 7.40%');
    expect(calc(CreditScore.fair).effectiveRate, closeTo(7.90, 0.001),
        reason: '7.90 + 0 = 7.90%');
    expect(calc(CreditScore.poor).effectiveRate, closeTo(9.90, 0.001),
  ...
  test('TEST 15 — UK petrolLarge VED £280/yr → £23.33/mo in payment', () {
    ...
    expect(r.vedTotal, closeTo(280.0 * 60 / 12, 0.01),
        reason: 'VED total = £280 × (60/12) = £1,400');

    // totalCost includes road tax
    expect(r.totalCost, closeTo(r.vehiclePrice + r.totalInterest + r.vedTotal, 0.01));
  });
}
