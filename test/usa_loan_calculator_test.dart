...
void main() {
  ...
  group('US — Calcul de base', () {
    ...
    setUpAll(() => r = USCalculation.calculate(
      vehiclePrice: 30000, tradeInValue: 0, downPayment: 3000,
      dealerFees: 500, salesTaxPercent: 8.0, annualRate: 6.9,
      termMonths: 60, creditScore: CreditScore.fair,
    ));

    test('[US-1a] Taxe = 8% × \$30,000 = \$2,400.00', () {
      expect(r.taxAmount, closeTo(2400.00, 0.01),
          reason: '[US-1a] 30000 × 8% = 2400');
    });

    test('[US-1b] Montant financé = prix + taxe + frais − down = \$29,900.00', () {
      expect(r.financedAmount, closeTo(29900.00, 0.01),
          reason: '[US-1b] 30000 + 2400 + 500 − 3000 = 29900');
    });

    test('[US-1c] Taux effectif Fair = 6.90% (pas d\'ajustement)', () {
      expect(r.effectiveRate, closeTo(6.90, 0.001),
    ...
    test('[US-1d] Paiement mensuel = PMT(29900, 6.9%/12, 60)', () {
      ...
      expect(r.monthlyPayment, closeTo(expected, 0.01),
          reason: '[US-1d] Formule PMT — spec dit \$583.63 mais formule donne ~\$590.57');
    });
    ...
    test('[US-1f] Coût total = prix + taxe + frais + intérêts − trade-in', () {
      ...
      expect(r.totalCost, closeTo(expected, 0.01),
          reason: '[US-1f] vehiclePrice + taxAmount + dealerFees + totalInterest − tradeIn');
    });
  });
  ...
  group('US — Credit Score', () {
    USCalculation calc(CreditScore cs) => USCalculation.calculate(
      vehiclePrice: 30000, tradeInValue: 0, downPayment: 3000,
      dealerFees: 500, salesTaxPercent: 8.0, annualRate: 6.9,
      termMonths: 60, creditScore: cs,
    );

    test('[US-2a] Excellent (750+) : taux − 1.5% → effectiveRate = 5.40%', () {
      expect(calc(CreditScore.excellent).effectiveRate, closeTo(5.40, 0.001),
    ...
    test('[US-3a] Good (700-749) : taux − 0.5% → effectiveRate = 6.40%', () {
      expect(calc(CreditScore.good).effectiveRate, closeTo(6.40, 0.001),
    ...
    test('[US-4a] Poor (<650) : taux + 2.0% → effectiveRate = 8.90%', () {
      expect(calc(CreditScore.poor).effectiveRate, closeTo(8.90, 0.001),
    ...
    test('[US-CS-ord] Excellent < Good < Fair < Poor (paiements croissants)', () {
    ...
    test('[US-CS-clamp] Taux plafonné à 30% (poor + taux de base 29%)', () {
      final r = USCalculation.calculate(
        vehiclePrice: 10000, tradeInValue: 0, downPayment: 0,
        dealerFees: 0, salesTaxPercent: 0, annualRate: 29.0,
        termMonths: 12, creditScore: CreditScore.poor, // +2% → 31% → clampé à 30%
      );
  ...
  group('US — Trade-in', () {
    test('[US-5a] Taxe = 6% × \$35,000 = \$2,100.00', () {
      final r = USCalculation.calculate(
        vehiclePrice: 35000, tradeInValue: 8000, downPayment: 2000,
        dealerFees: 800, salesTaxPercent: 6.0, annualRate: 5.9,
        termMonths: 48, creditScore: CreditScore.fair,
      );
      expect(r.taxAmount, closeTo(2100.00, 0.01),
    ...
    test('[US-5b] Montant financé avec trade-in = \$27,900.00', () {
      ...
      expect(r.financedAmount, closeTo(27900.00, 0.01),
          reason: '[US-5b] 35000 + 2100 + 800 − 8000 − 2000 = 27900');
    });

    test('[US-5c] Paiement mensuel avec trade-in = PMT(27900, 5.9%/12, 48)', () {
      final r = USCalculation.calculate(
        vehiclePrice: 35000, tradeInValue: 8000, downPayment: 2000,
        dealerFees: 800, salesTaxPercent: 6.0, annualRate: 5.9,
        termMonths: 48, creditScore: CreditScore.fair,
    ...
    test('[US-5d] Trade-in réduit le montant financé de \$8,000', () {
      final noTrade = USCalculation.calculate(
        vehiclePrice: 35000, tradeInValue: 0, downPayment: 2000,
        dealerFees: 800, salesTaxPercent: 6.0, annualRate: 5.9,
        termMonths: 48, creditScore: CreditScore.fair,
      );
    ...
    test('[US-5e] Coût total = prix + taxe + frais + intérêts − trade-in', () {
      ...
      expect(r.totalCost, closeTo(expected, 0.01),
          reason: '[US-5e] totalCost = vehiclePrice + tax + fees + interest − tradeIn');
    });
  });
  ...
  group('US — Amortissement', () {
    ...
    setUpAll(() {
      calc = USCalculation.calculate(
        vehiclePrice: 30000, tradeInValue: 0, downPayment: 3000,
        dealerFees: 500, salesTaxPercent: 8.0, annualRate: rate,
        termMonths: term, creditScore: CreditScore.fair,
      );
      rows = buildSchedule(loanAmount: financed, annualRate: rate, termMonths: term);
    });
    ...
    test('[US-6b] Somme des principaux = \$29,900.00 ± \$1.00', () {
      ...
      expect(sum, closeTo(financed, 1.00),
    ...
    test('[US-6e] Taux effectif affiché = taux après ajustement credit score', () {
      expect(calc.effectiveRate, closeTo(6.90, 0.001),
  ...
  group('US — Sales Tax = 0%', () {
    test('[US-7a] Taxe 0% → taxAmount = \$0', () {
      final r = USCalculation.calculate(
        vehiclePrice: 20000, tradeInValue: 0, downPayment: 2000,
        dealerFees: 300, salesTaxPercent: 0.0, annualRate: 6.9,
        termMonths: 60, creditScore: CreditScore.fair,
      );
      expect(r.taxAmount, closeTo(0.0, 0.001),
    ...
    test('[US-7b] Taxe 0% → financedAmount = prix + frais − down − trade', () {
      ...
      expect(r.financedAmount, closeTo(20000 + 0 + 300 - 0 - 2000, 0.01),
  ...
  group('Cas limites US', () {
    ...
    test('[US-8a] Trade-in > Down Payment — financedAmount réduit', () {
      final noTrade = USCalculation.calculate(
        vehiclePrice: 25000, tradeInValue: 0, downPayment: 2000,
        dealerFees: 0, salesTaxPercent: 0, annualRate: 6.9,
        termMonths: 60, creditScore: CreditScore.fair,
      );
      final withTrade = USCalculation.calculate(
        vehiclePrice: 25000, tradeInValue: 10000, downPayment: 2000,
        dealerFees: 0, salesTaxPercent: 0, annualRate: 6.9,
        termMonths: 60, creditScore: CreditScore.fair,
      );
      expect(withTrade.financedAmount,
    ...
    test('[US-8b] Down Payment = Prix → financedAmount minimal (juste taxe + frais)', () {
      const price = 20000.0;
      final r = USCalculation.calculate(
        vehiclePrice: price, tradeInValue: 0, downPayment: price,
        dealerFees: 300, salesTaxPercent: 8.0, annualRate: 6.9,
        termMonths: 60, creditScore: CreditScore.fair,
      );
      // financed = price + tax + fees - tradeIn - down = price×1.08 + 300 - price = price×0.08 + 300
      final expectedFinanced = price * 0.08 + 300.0;
      expect(r.financedAmount, closeTo(expectedFinanced, 0.01),
          reason: '[US-8b] financed = taxe + frais seulement');
    });

    test('[US-8c] Down Payment excessif → financedAmount clampé à 0', () {
      final r = USCalculation.calculate(
        vehiclePrice: 10000, tradeInValue: 0, downPayment: 99999,
        dealerFees: 0, salesTaxPercent: 0, annualRate: 6.9,
        termMonths: 60, creditScore: CreditScore.fair,
      );
      expect(r.financedAmount, 0.0,
    ...
    test('[US-8d] Taux 0.99% — paiement proche remboursement simple', () {
      final r = USCalculation.calculate(
        vehiclePrice: 30000, tradeInValue: 0, downPayment: 3000,
        dealerFees: 0, salesTaxPercent: 0, annualRate: 0.99,
        termMonths: 60, creditScore: CreditScore.fair,
      );
    ...
    test('[US-8e] Taux 24.99% — intérêts très élevés', () {
      final r = USCalculation.calculate(
        vehiclePrice: 30000, tradeInValue: 0, downPayment: 3000,
        dealerFees: 0, salesTaxPercent: 0, annualRate: 24.99,
        termMonths: 60, creditScore: CreditScore.fair,
      );
      expect(r.monthlyPayment,
          closeTo(_pmt(27000.0, 24.99 / 12 / 100, 60), 0.01),
          reason: '[US-8e] PMT(27000, 24.99%/12, 60)');
      expect(r.totalInterest, greaterThan(r.financedAmount * 0.7),
    ...
    test('[US-8f] Durée 24 mois — paiement mensuel', () {
      final r = USCalculation.calculate(
        vehiclePrice: 30000, tradeInValue: 0, downPayment: 3000,
        dealerFees: 500, salesTaxPercent: 8.0, annualRate: 6.9,
        termMonths: 24, creditScore: CreditScore.fair,
      );
      expect(r.monthlyPayment,
    ...
    test('[US-8g] Durée 84 mois — paiement plus bas que 24 mois', () {
      ...
      final r84 = USCalculation.calculate(
        vehiclePrice: 30000, tradeInValue: 0, downPayment: 3000,
        dealerFees: 500, salesTaxPercent: 8.0, annualRate: 6.9,
        termMonths: 84, creditScore: CreditScore.fair,
      );
    ...
    test('[US-8h] Taux 0% — paiement = financedAmount / terme', () {
      final r = USCalculation.calculate(
        vehiclePrice: 30000, tradeInValue: 0, downPayment: 3000,
        dealerFees: 0, salesTaxPercent: 0, annualRate: 0,
        termMonths: 60, creditScore: CreditScore.fair,
      );
      expect(r.totalInterest, 0.0, reason: '[US-8h] Pas d\'intérêt à 0%');
      expect(r.monthlyPayment, closeTo(27000.0 / 60, 0.01),
  ...
  group('US — Bi-weekly', () {
    ...
    setUpAll(() => r = USCalculation.calculate(
      vehiclePrice: 30000, tradeInValue: 0, downPayment: 3000,
      dealerFees: 500, salesTaxPercent: 8.0, annualRate: 6.9,
      termMonths: termMonths, creditScore: CreditScore.fair,
      isBiWeekly: true,
    ));
    ...
    test('[US-9d] Bi-weekly ≠ mensuel × 12/26 (formules indépendantes)', () {
      ...
      expect(r.biWeeklyPayment, isNot(closeTo(approx, 0.01)),
    ...
    test('[US-9e] displayPayment mensuel quand isBiWeekly=false', () {
      ...
      expect(monthly.displayPayment, closeTo(monthly.monthlyPayment, 0.001),
    ...
    test('[US-9f] Taux 0% bi-weekly = financed / (years×26)', () {
      ...
      expect(r0.biWeeklyPayment, closeTo(27000.0 / (5 * 26), 0.01),
