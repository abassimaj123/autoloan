...
void main() {
  ...
  group('CA — Calcul de base', () {
    ...
    test('[CA-1g] Mois 1 : principal ≈ \$394.36', () {
      ...
      expect(principal1, closeTo(394.36, 0.10),
    ...
    test('[CA-1h] Mois 1 : balance ≈ \$28,505.64', () {
      ...
      expect(balance1, closeTo(28505.64, 0.50),
  ...
  group('CA — Assurances', () {
    ...
    test('[CA-2a] Assurance mensuelle = vie + garantie/60 + GAP/60 = \$32.50', () {
      expect(insMonthly, closeTo(32.50, 0.01),
          reason: '[CA-2a] 20 + 8.33 + 4.17 = 32.50/mo (spec dit 28.33 — erreur arithmétique)');
    });
    ...
    test('[CA-2c] Total assurances = 20×60 + 500 + 250 = \$1,950.00', () {
      ...
      expect(r.insuranceTotal, closeTo(1950.00, 0.01),
    ...
    test('[CA-2d] Coût total avec assurances ≈ \$42,026.34', () {
      ...
      final expected = 30000.0 + r.taxAmount + r.totalInterest + r.insuranceTotal;
      expect(r.totalCost, closeTo(expected, 0.01),
          reason: '[CA-2d] prix + taxe + intérêts + assurances');
      expect(r.totalCost, closeTo(42026.34, 1.00),
  ...
  group('CA — Bi-weekly', () {
    ...
    test('[CA-3b] Taux bi-weekly = 7.90% / 26 = 0.30385%', () {
      ...
      expect(rBi, closeTo(0.003038, 0.000001),
    ...
    test('[CA-3c] Paiement bi-weekly calculé avec formule indépendante', () {
      ...
      expect(r.baseLoanBiWeekly, closeTo(expected, 0.01),
      ...
      expect(r.baseLoanBiWeekly, inInclusiveRange(267.0, 272.0),
          reason: '[CA-3c] Valeur attendue entre \$267 et \$272');
    });

    test('[CA-3d] Bi-weekly ≠ mensuel × 12/26 (formules indépendantes — bug #2 corrigé)', () {
      final r = CACalculation.calculate(
        vehiclePrice: 30000, downPayment: 5000, annualRate: 7.9,
        termMonths: 60, provinceCode: 'ON', isBiWeekly: true,
      );
      final converted = r.baseLoanMonthly * 12 / 26;
      expect(r.baseLoanBiWeekly, isNot(closeTo(converted, 0.01)),
          reason: '[CA-3d] Formules indépendantes — bi-weekly ≠ monthly×12/26');
    });

    test('[CA-3e] Équivalence annuelle : bi-weekly×26 vs mensuel×12 (légère diff attendue)', () {
      final r = CACalculation.calculate(
        vehiclePrice: 30000, downPayment: 5000, annualRate: 7.9,
        termMonths: 60, provinceCode: 'ON', isBiWeekly: true,
      );
      ...
      expect((annualBi - annualMo).abs(), lessThan(15.0),
          reason: '[CA-3e] Différence annuelle < \$15 (formules proches mais indépendantes)');
    });
    ...
    test('[CA-3f] Assurance bi-weekly = mensuel × 12/26', () {
      ...
      expect(r.insuranceBiWeekly, closeTo(ins * 12 / 26, 0.0001),
  ...
  group('CA — Province QC', () {
    ...
    test('[CA-4a] Taxe QC = 14.975% × \$30,000 = \$4,492.50', () {
      expect(r.taxAmount, closeTo(4492.50, 0.01),
          reason: '[CA-4a] QC GST(5%) + PST(9.975%) = 14.975%');
    });

    test('[CA-4b] Montant financé QC = \$29,492.50', () {
      expect(r.loanAmount, closeTo(29492.50, 0.01),
    ...
    test('[CA-4c] Paiement mensuel QC = PMT(29492.50, 7.9%/12, 60)', () {
      ...
      expect(r.monthlyPayment, greaterThan(590.0),
          reason: '[CA-4c] Plus grand que CA-1 car capital QC plus élevé');
    });

    test('[CA-4d] Intérêts totaux QC = pmt×60 − 29492.50', () {
      expect(r.totalInterest, closeTo(r.monthlyPayment * 60 - 29492.50, 0.01),
  ...
  group('CA — Provinces', () {
    ...
    void testProv(String code, double rate, double expectedTax) {
      test('[$code] taux=${(rate * 100).toStringAsFixed(3)}% → taxe=\$${expectedTax.toStringAsFixed(2)}', () {
        final prov = caProvinceByCode(code);
        expect(prov.totalRate, closeTo(rate, 0.00001),
            reason: '[$code] totalRate = $rate');
        final r = CACalculation.calculate(
          vehiclePrice: price, downPayment: 0, annualRate: 0,
          termMonths: 12, provinceCode: code, isBiWeekly: false,
        );
        expect(r.taxAmount, closeTo(expectedTax, 0.01),
            reason: '[$code] taxe = $price × ${(rate * 100).toStringAsFixed(3)}%');
      });
    }
  ...
  group('CA — Amortissement', () {
    ...
    setUpAll(() {
      calc = CACalculation.calculate(
        vehiclePrice: 30000, downPayment: 5000, annualRate: rate,
        termMonths: term, provinceCode: 'ON', isBiWeekly: false,
      );
    ...
    test('[CA-5c] Somme des intérêts ≈ \$6,176.34 ± \$1.00', () {
      ...
      expect(sum, closeTo(6176.34, 1.00),
          reason: '[CA-5c] Spec ≈ \$6,176.34');
    });
    ...
    test('[CA-5e] Header Total Cost = Σ paiements + mise de fonds (\$5,000)', () {
      final sumPayments = rows.fold(0.0, (s, r) => s + r.payment);
      final headerTotal = sumPayments + 5000.0;
      expect(headerTotal, closeTo(calc.totalCost, 1.00),
          reason: '[CA-5e] Σ paiements + down ≈ totalCost');
    });

    test('[CA-5f] Chaque ligne : balance = balance_prev − principal (± \$0.02)', () {
      double balance = P;
      for (final row in rows) {
        final newBalance = (balance - row.principal).clamp(0.0, double.infinity);
        expect(row.balance, closeTo(newBalance, 0.02),
    ...
    test('[CA-5g] Mois 1 : intérêt = 28900 × 7.9%/12 ≈ \$190.25', () {
      expect(rows.first.interest, closeTo(190.25, 0.10),
  ...
  group('Cas limites CA', () {
    ...
    test('[CA-7] Mise de fonds % : 20% de \$40,000 = \$8,000', () {
      ...
      expect(dpAmount, closeTo(8000.00, 0.01),
          reason: '[CA-7] 40000 × 20% = 8000');
      final r = CACalculation.calculate(
        vehiclePrice: vehiclePrice, downPayment: dpAmount,
        annualRate: 7.9, termMonths: 60, provinceCode: 'ON', isBiWeekly: false,
      );
      expect(r.downPayment, closeTo(8000.00, 0.01),
          reason: '[CA-7] downPayment stocké = 8000');
    });

    test('[CA-8a] Mise de fonds = \$0 → financé = prix + taxe', () {
      final r = CACalculation.calculate(
        vehiclePrice: 30000, downPayment: 0, annualRate: 7.9,
        termMonths: 60, provinceCode: 'ON', isBiWeekly: false,
      );
      expect(r.loanAmount, closeTo(30000 * 1.13, 0.01),
          reason: '[CA-8a] 30000 + 13% = 33900');
    });

    test('[CA-8b] Mise de fonds = prix → loan = taxe seulement', () {
      final r = CACalculation.calculate(
        vehiclePrice: 30000, downPayment: 30000, annualRate: 7.9,
        termMonths: 60, provinceCode: 'ON', isBiWeekly: false,
      );
      expect(r.loanAmount, closeTo(30000 * 0.13, 0.01),
    ...
    test('[CA-8c] Durée 24 mois — paiement mensuel correct', () {
      final r = CACalculation.calculate(
        vehiclePrice: 30000, downPayment: 5000, annualRate: 7.9,
        termMonths: 24, provinceCode: 'ON', isBiWeekly: false,
      );
    ...
    test('[CA-8d] Durée 84 mois — paiement plus petit que 24 mois', () {
      final r84 = CACalculation.calculate(
        vehiclePrice: 30000, downPayment: 5000, annualRate: 7.9,
        termMonths: 84, provinceCode: 'ON', isBiWeekly: false,
      );
      final r24 = CACalculation.calculate(
        vehiclePrice: 30000, downPayment: 5000, annualRate: 7.9,
        termMonths: 24, provinceCode: 'ON', isBiWeekly: false,
      );
      expect(r84.monthlyPayment, closeTo(_pmt(28900.0, 7.9 / 12 / 100, 84), 0.01),
          reason: '[CA-8d] PMT(28900, 7.9%/12, 84)');
      expect(r84.monthlyPayment, lessThan(r24.monthlyPayment),
          reason: '[CA-8d] 7 ans → pmt < 2 ans');
    });

    test('[CA-8e] Taux 0.99% — intérêts positifs faibles', () {
      final r = CACalculation.calculate(
        vehiclePrice: 30000, downPayment: 5000, annualRate: 0.99,
        termMonths: 60, provinceCode: 'ON', isBiWeekly: false,
      );
      expect(r.totalInterest, greaterThan(0),
          reason: '[CA-8e] Intérêts > 0 même à 0.99%');
      expect(r.monthlyPayment, closeTo(_pmt(28900.0, 0.99 / 12 / 100, 60), 0.01),
          reason: '[CA-8e] PMT(28900, 0.99%/12, 60)');
    });

    test('[CA-8f] Taux 19.99% — intérêts élevés', () {
      final r = CACalculation.calculate(
        vehiclePrice: 30000, downPayment: 5000, annualRate: 19.99,
        termMonths: 60, provinceCode: 'ON', isBiWeekly: false,
      );
      expect(r.monthlyPayment, closeTo(_pmt(28900.0, 19.99 / 12 / 100, 60), 0.01),
    ...
    test('[CA-8g] Mise de fonds excessive → loanAmount = 0', () {
      final r = CACalculation.calculate(
        vehiclePrice: 10000, downPayment: 99999, annualRate: 7.9,
        termMonths: 60, provinceCode: 'ON', isBiWeekly: false,
      );
      expect(r.loanAmount, 0.0,
          reason: '[CA-8g] loanAmount clampé à 0');
      expect(r.monthlyPayment, 0.0,
          reason: '[CA-8g] pmt = 0 si loan = 0');
    });

    test('[CA-8h] Taux 0% — paiement = loanAmount / terme', () {
      final r = CACalculation.calculate(
        vehiclePrice: 30000, downPayment: 5000, annualRate: 0,
        termMonths: 60, provinceCode: 'ON', isBiWeekly: false,
      );
