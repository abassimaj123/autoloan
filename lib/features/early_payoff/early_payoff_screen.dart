...
class _EarlyPayoffScreenState extends State<EarlyPayoffScreen> {
  ...
  _PayoffResult _compute(double extraPayment) {
    ...
    if (rate <= 0) {
      stdPayment = widget.termMonths > 0 ? widget.loanAmount / widget.termMonths : 0;
    } else {
    ...
    final double stdInterest = (stdPayment * widget.termMonths - widget.loanAmount)
    ...
    while (balance > 0 && periods < widget.termMonths * 2) {
      ...
      if (principal <= 0) break; // rate so high extra payment doesn't cover interest
      balance -= principal;
      periods++;
    ...
    final interestSaved = stdInterest - earlyInterest.clamp(0.0, double.infinity);
  ...
  Widget build(BuildContext context) {
    ...
    final fmt    = NumberFormat.currency(symbol: widget.currencySymbol, decimalDigits: 2);
          ...
          SectionCard(
            ...
            children: [
              ...
              ResultTile(
                ...
                value: '${widget.termMonths} mo (${widget.termMonths ~/ 12} yr)',
              ),
              ResultTile(
            ...
            SectionCard(
              ...
              children: [
                ResultTile(
                  ...
                  value: fmt.format(result.stdMonthlyPayment + _extraMonthly),
                  isHighlight: true,
                ),
                ResultTile(
                  label: 'Paid Off In',
                  value: '${result.earlyMonths} mo (${(result.earlyMonths / 12).toStringAsFixed(1)} yr)',
                ),
                ResultTile(
